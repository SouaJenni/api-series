...
// GET /filmes/:id
    async getFilmePorId(req, res) {
        try {
            const {id} = req.params;
            const filme = await filmeService.buscarFilmePorId(id);
            if (!filme) {
...
// POST /filmes
    async criarFilme(req, res) {
        try {
            const novoFilme = await filmeService.criarFilme(req.body);
...
// PUT /filmes/:id
    async atualizarFilme(req, res) {
        try {
            const {id} = req.params;
            const atualizado = await filmeService.atualizarFilme(id, req.body);
            if (!atualizado) {
...
// DELETE /filmes/:id
    async deletarFilme(req, res) {
        try {
            const {id} = req.params;
            const removido = await filmeService.deletarFilme(id);
            if (!removido) {
...
// GET /filmes/sugestoes/busca?q=nome
    async buscarSugestoes(req, res) {
        try {
            ...
            const sugestoes = await filmeService.buscarSugestoesExternas(q);
